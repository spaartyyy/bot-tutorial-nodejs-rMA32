...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      ...
      botHamad = /^\/Hamadsdad/; botRing = /^\/ring/; botKeith = /^\/keith/; botHi = /^\/hi/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/lg/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/lg/team/"+request.text.substring(5,8)+"/schedule");
    this.res.end();
  }
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/lg/players?name="+rep+"&position=all&team=all");
    this.res.end();
  }  
  ...
  else if(request.text && botRing.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://i.imgur.com/EKHM1GFh.jpg");
    this.res.end();
  } 
  ...
  else if(request.text && botDE.test(request.text)) {
    this.res.writeHead(200);
    postMessage("hes gay");
    this.res.end();
  } 
  ...
  else if(request.text && botHi.test(request.text)) {
    this.res.writeHead(200);
    postMessage("http://i.imgur.com/qt89fRt.gif");
    this.res.end();
  }
  else {
